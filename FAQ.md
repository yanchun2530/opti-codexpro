...
Opti-CodexPro does not unlock Plugins, unlock models, bypass account limits, or provide account access. It connects to the ChatGPT plugin surface your account already has.

Plan access and model tool support are separate, and availability can change. If Opti-CodexPro actions are unavailable in that chat, use another tool-capable ChatGPT surface or the Pro context fallback for that session.

## How is Opti-CodexPro different from generic workspace bridges?

They can look similar at the transport layer because both use a local MCP-style bridge and a workspace root.

Opti-CodexPro is built around one product loop:
...
- Opti-CodexPro is ChatGPT Plugins + MCP first, not a generic workspace bridge.
...
- Opti-CodexPro keeps a strict boundary: no model proxying, account pooling, third-party Pro site scraping, quota bypassing, or OS sandbox claims.

Opti-CodexPro connects ChatGPT to a user-approved local repository over MCP. Repository access, command permissions, and change review remain explicit.
...
Relationships are labeled `exact`, `strong`, or `inferred`. The repository map does not replace a compiler or language server. Opti-CodexPro does not require a language server, daemon, embedding service, or vector database.

Analysis is process-local and cached by a bounded workspace fingerprint. Direct Opti-CodexPro writes, edits, and patches invalidate that cache. If limits are reached, results say `partial` and retain normal tree/search/read/review fallback behavior.
...
## How is Opti-CodexPro different from ChatGPT's built-in web Agent?
...
Opti-CodexPro is a local MCP bridge: your ChatGPT session talks to an approved folder on your computer through Plugins. Developer mode is only the ChatGPT settings toggle that lets you create custom plugins. It does not replace the web Agent, bypass account limits, or turn ChatGPT into a remote shell service.

Use the web Agent for web work. Use Opti-CodexPro when the source of truth is a local repository.

## How do I import a ChatGPT attachment into my repo?

In workspace write mode, Opti-CodexPro advertises `import_file`. ChatGPT must pass an Apps SDK file object:
...
Opti-CodexPro marks that argument with `_meta["openai/fileParams"]`. It downloads only temporary HTTPS URLs from approved ChatGPT/OpenAI file hosts, enforces `CODEXPRO_MAX_IMPORT_BYTES`, rejects private/loopback redirect targets, and writes into the allowed workspace only. Overwrite defaults to false. Arbitrary user- or model-supplied download URLs are rejected.
...
Server URL: paste the URL copied by Opti-CodexPro
Authentication: No Authentication / None
```

The copied Server URL already includes the private Opti-CodexPro token.

## Should CSP stay enabled?

Yes. Keep Enforce CSP in developer mode enabled.

Opti-CodexPro widgets are built for the CSP-enabled path. They do not need unrestricted network access, external fonts, remote scripts, iframes, or third-party images.

## Does Opti-CodexPro bypass rate limits?

No.

Opti-CodexPro does not bypass, avoid, increase, pool, resell, or modify ChatGPT, Codex, OpenAI, or third-party model limits. Every request still runs through the user's own ChatGPT session and whatever limits that account has.

The useful part is that Codex and ChatGPT are different product surfaces. If one workflow is unavailable and another product surface you already have access to is still available, Opti-CodexPro lets you work against the same local repo without changing either product's limits.

## Can Opti-CodexPro use GPT-5.5?
...
Some GPT-5.5 Pro or other model surfaces may not expose plugin actions in a given chat. If Opti-CodexPro actions are unavailable there, Opti-CodexPro cannot make that request reach the local server. Opti-CodexPro does not provide, proxy, resell, or unlock models. It gives compatible ChatGPT sessions local repo tools.
...
## What can ChatGPT see through Opti-CodexPro?
...
## Can Opti-CodexPro bind bash to a specific session id?

Opti-CodexPro cannot attach to, read, or execute inside a specific Codex app conversation or terminal session.

The MCP `bash` tool runs from the Opti-CodexPro server process you started for the configured workspace. MCP session ids are HTTP transport state between ChatGPT and Opti-CodexPro; they are not Codex conversation ids.

What Opti-CodexPro can do is require a matching local bash session label before it runs shell commands:

```bash
codexpro start --bash-session main --require-bash-session
```

Then `bash` calls must include `session_id: "main"`. This helps avoid accidental shell execution in the wrong Opti-CodexPro terminal, but it is not remote control of an existing Codex app chat.

Opti-CodexPro can list local Codex session ids and titles when you explicitly opt in:
...
If you do not want ChatGPT to trigger shell commands while you work in Codex, start Opti-CodexPro with bash disabled:

```bash
codexpro start --no-bash
```

This removes the `bash` MCP tool from the advertised tool list. ChatGPT can still use non-bash Opti-CodexPro tools such as workspace open, read, search, and show_changes. Direct `write`/`edit` are advertised only in workspace write mode.
...
- No `POST /mcp received`: the request did not reach Opti-CodexPro. Check the ChatGPT
  Plugins page and the tunnel.
- `POST /mcp -> 401`: paste the complete URL, including `codexpro_token`.
- `POST /mcp -> 2xx`: ChatGPT reached Opti-CodexPro and the MCP endpoint responded.
...
OAuth or `Authorization: Bearer <token>`. Opti-CodexPro
...
Keep Opti-CodexPro running while testing. A Cloudflare quick-tunnel URL changes on
...
Opti-CodexPro can handle an MCP request. Remove or recreate the stale plugin entry
from the Plugins page, then retry with the current URL. Opti-CodexPro cannot repair
...
The same hostname and Opti-CodexPro token are reused for that workspace.

## What if I run Opti-CodexPro in two repos at once?
...
Workspace selection is isolated between MCP sessions created by the client. A ChatGPT conversation is not guaranteed to map one-to-one to an MCP session, so use separate Opti-CodexPro processes when strict isolation matters.

For separate processes, two ChatGPT accounts, or two ngrok domains on one machine, run two Opti-CodexPro processes with different local ports and different public hostnames:
...
Workspace selection is session-local. For shared files, read the file first and pass its returned SHA-256 as `expected_sha256` to `write` or `edit`. Opti-CodexPro rejects the operation if the file changed after that read. New files use atomic replacement; existing files are updated in place to retain inode-bound metadata and hard links.

This protects against stale file content. It does not turn Opti-CodexPro into a collaborative merge server, so separate worktrees remain the stronger choice for large overlapping changes.
...
## Is Opti-CodexPro production safe?

Opti-CodexPro is a local developer bridge, not an OS sandbox.
...
Opti-CodexPro stores local state under `~/.codexpro` by default. On Windows that is usually `C:\Users\<you>\.codexpro`.
