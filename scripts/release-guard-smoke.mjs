...
const npmProbe = npmCli ? { status: 0 } : spawnSync(npm, ["--version"], { encoding: "utf8" });
const npmAvailable = npmCli || npmProbe.status === 0;
...
try {
  ...
  assert.match(allowed.output, /Opti-CodexPro release guard: opti-codexpro@\d+\.\d+\.\d+/);
  ...
  assert.match(wrongDirectory.output, /Release commands must run from the Opti-CodexPro root/);
  ...
  if (npmAvailable) {
    ...
    assert.match(prefixInvocation.output, /Release commands must run from the Opti-CodexPro root/);
    ...
    console.log("release-guard-smoke: npm unavailable; skipped npm-prefix and pack assertions");
  }
} finally {
