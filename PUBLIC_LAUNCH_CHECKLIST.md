...
Opti-CodexPro is a local developer bridge. Treat public launch readiness as two separate gates:
...
Do not present Opti-CodexPro as a fully reviewed public ChatGPT app until it has gone through the current app review flow.
...
```text
Use Opti-CodexPro. Call server_config, then open_current_workspace with include_tree=false. Read README.md and summarize the project without editing files.
```

```text
Use Opti-CodexPro. Create a small static site from PRODUCT.md by writing index.html, styles.css, and README.md. Verify with one targeted search.
```

```text
Use Opti-CodexPro. Try to read .env. Explain why the request is blocked.
```

```text
Use Opti-CodexPro. Run bash with pwd, then run bash with a blocked command. Report both outcomes.
```
...
- Opti-CodexPro is not an OS sandbox.
- Opti-CodexPro does not guarantee a ChatGPT model can call MCP tools.
- Opti-CodexPro does not change ChatGPT, Codex, or OpenAI quota behavior.
